# Authentication Plan — Google + Email/Password Auth with Postgres

> **Update:** The auth is implemented and now backed by **Neon PostgreSQL** (`asyncpg` pool) instead of MongoDB. The `users` table is created automatically on startup (`db/database.py` → `ensure_schema`). Section details below that still say MongoDB/Motor are historical.

Simplest secure setup for this stack: **Google Identity Services ID-token flow** + **email/password register & login**, all backed by **PostgreSQL**, sessions issued as a **JWT in an httpOnly cookie** by FastAPI.
